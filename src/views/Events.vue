  ...
  <v-container class="" style="max-width: 1600px">
      ...
      <v-col>
        ...
        >
          <v-toolbar-title class="google-font mr-3"
            >Events | {{ generalConfig.name || 'Aura' }}</v-toolbar-title
          >
          <v-spacer></v-spacer>
    ...
    <v-row>
      <v-col>
        <UpcomingMeetupEvents
          v-if="keysandsecurity.meetup.length > 3"
          :meetupkey="keysandsecurity.meetup"
        />
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col
        cols="12"
        sm="12"
        :md="keysandsecurity.meetup.length > 3 ? '5' : '12'"
        class=""
      >
        <CustomEvents />
      </v-col>
...
export default {
  ...
  components: {
    ...
    UpcomingMeetupEvents: () =>
      import('@/components/Events/Meetups/UpcomingEvents'),
    CustomEvents: () => import('@/components/Events/CustomEvents/CustomEvents'),
    Snakebar: () => import('@/components/Common/Snakebar'),
    FeatureEvents: () =>
      import('@/components/Events/FeaturesEvents/FeatureEvents'),
  },
  data: () => ({
    userEmail: '',
    snakeBarMessage: '',
