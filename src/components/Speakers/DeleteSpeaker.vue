  ...
  <v-dialog v-model="dialog" max-width="500">
    ...
    <v-card v-if="dialog">
      ...
      <v-card-text class="google-font"
        >Would you like to remove {{ SpeakerInfo.name }} form the
        Speakers?</v-card-text
      >
      <v-card-actions>
        ...
        <v-btn
          ...
          text
          :loading="loading"
          @click="deleteItem(SpeakerInfo.id)"
          >Agree</v-btn
        >
      </v-card-actions>
    </v-card>
...
export default {
  ...
  methods: {
    deleteItem(id) {
      this.loading = true;
      SpeakerServices.removeSpeaker(this.SpeakerInfo.id)
        .then((res) => {
          if (res.success == true) {
            this.loading = false;
            this.dialog = false;
